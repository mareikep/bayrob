...
from calo.models.world import Agent
...
class JointProbabilityTreesMPE(unittest.TestCase):
    ...
    def test_robot_pos_random(self):
        dt = f'{datetime.datetime.now().strftime(FILESTRFMT)}'
        ...
        for j in range(100):
            ...
            for action in range(100):
                ...
                step = randint(1, 10)
                poses.append(a.pos+a.dir+(step,))
                Move.moveforward(a, step)
    ...
    def test_data_curation(self):
        ...
        data_moveforward.to_csv(os.path.join(locs.logs, f'{dt}-{"ALL"}-{"MOVEFORWARD"}.csv'), index=False)
    ...
    def test_jpt_moveforward(self):
        ...
        dt = '2022-08-19_12:15'
        data_moveforward = pd.read_csv(os.path.join(locs.logs, f'{dt}-ALL-MOVEFORWARD.csv'), delimiter=',', header=0)
        ...
        jpt_moveforward.plot(title='MOVEFORWARD', plotvars=movevars, filename=f'{dt}-ALL-MOVEFORWARD', directory=locs.logs)
    ...
    def test_calo(self):
        ...
        dt = '2022-08-19_12:15'
        jpt = calo.models[f'dt-ALL-MOVEFORWARD.tree']
