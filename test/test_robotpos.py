...
from core import CALO
from jpt import infer_from_dataframe, JPT
from jpt.distributions import Gaussian
from models.action import TrajectorySimulation, Move
from models.world import Agent
from utils import locs
from utils import FILESTRFMT
...
class JointProbabilityTreesMPE(unittest.TestCase):
    ...
    def test_data_curation(self):
        ...
        dt = '2022-08-04_09:59-{}-{}.csv'
        data_moveforward = pd.DataFrame(columns=['deltax', 'deltay', 'xdir', 'ydir', 'numsteps'])
        for i in range(100):
            with open(os.path.join(locs.logs, dt.format(i, 'MOVEFORWARD')), 'r') as f:
        ...
        data_moveforward.to_csv(os.path.join(locs.logs, dt.format('ALL', 'MOVEFORWARD')), index=False)
        
        data_turn = pd.DataFrame(columns=['deltaxdir', 'deltaydir', 'angle'])
        for i in range(100):
            with open(os.path.join(locs.logs, dt.format(i, 'TURN')), 'r') as f:
                d = pd.read_csv(f, delimiter=',', header=0)
                for idx, row in d.iterrows():
                    if idx == d.index.max(): break
                    data_turn.loc[idx+i*100] = [d.iloc[idx+1]['xdir']-row['xdir'], d.iloc[idx+1]['ydir']-row['ydir'], row['angle']]
        data_turn.to_csv(os.path.join(locs.logs, dt.format('ALL', 'TURN')), index=False)

    def test_jpt_moveforward(self):
        # learn discriminative JPT from data generated by test_data_curation for MOVEFORWARD
        data_moveforward = pd.read_csv(os.path.join(locs.logs, '2022-08-04_09:59-ALL-MOVEFORWARD.csv'), delimiter=',', header=0)
        movevars = infer_from_dataframe(data_moveforward)
        jpt_moveforward = JPT(variables=movevars, targets=movevars[:2], min_samples_leaf=.05)
        jpt_moveforward.learn(columns=data_moveforward.values.T)
        jpt_moveforward.plot(title='MOVEFORWARD', plotvars=movevars, filename='2022-08-04_09:59-ALL-MOVEFORWARD.svg', directory=locs.logs)
        jpt_moveforward.save(os.path.join(locs.logs, '2022-08-04_09:59-ALL-MOVEFORWARD.tree'))

    def test_jpt_turn(self):
        # learn discriminative JPT from data generated by test_data_curation for TURN
        data_turn = pd.read_csv(os.path.join(locs.logs, '2022-08-04_09:59-ALL-TURN.csv'), delimiter=',', header=0)
        turnvars = infer_from_dataframe(data_turn)
        jpt_turn = JPT(variables=turnvars, targets=turnvars[:2], min_samples_leaf=.01)
        jpt_turn.learn(columns=data_turn.values.T)
        jpt_turn.plot(title='TURN', plotvars=turnvars, filename='2022-08-04_09:59-ALL-TURN.svg', directory=locs.logs)
        jpt_turn.save(os.path.join(locs.logs, '2022-08-04_09:59-ALL-TURN.tree'))
    ...
    def test_calo(self):
        ...
        jpt = calo.models['2022-08-04_09:59-ALL-MOVEFORWARD.tree']
