...
def data_curation(dt, linear=False):
            ...
            for idx, row in d.iterrows():
                ...
                if COLLIDED:
                    data_moveforward.loc[idx + i * RUNS] = [
                        ...
                        row['x'],
                        row['y'],
                        d.iloc[idx + 1]['x']-row['x'] if linear else d.iloc[idx + 1]['x'],
                        d.iloc[idx + 1]['y']-row['y'] if linear else d.iloc[idx + 1]['y'],
                        row['collided']
                    ]
                else:
                    data_moveforward.loc[idx + i * RUNS] = [
        ...
        with open(os.path.join(locs.examples, 'robotaction', dt, 'data', f'{i}-TURN.csv'), 'r') as f:
            ...
            for idx, row in d.iterrows():
                ...
                data_turn.loc[idx + i * RUNS] = [
                    row['xdir'],
                    row['ydir'],
                    d.iloc[idx + 1]['xdir']-row['xdir'] if linear else d.iloc[idx + 1]['xdir'],
                    d.iloc[idx + 1]['ydir']-row['ydir'] if linear else d.iloc[idx + 1]['ydir'],
                    row['angle']
                ]
...
def learn_jpt_moveforward(dt):
    ...
    logger.debug(f'...done! saving to file {os.path.join(locs.examples, "robotaction", dt, f"MOVEFORWARD.tree")}')
...
def learn_jpt_turn(dt):
    ...
    logger.debug(f'...done! saving to file {os.path.join(locs.examples, "robotaction", dt, f"TURN.tree")}')
...
if __name__ == '__main__':
    ...
    if SEMI:
        ...
        # robot_pos_random(DT)
        data_curation(DT, linear=LINEAR)
