/* _src/pages/setup.typ */
#include "/_src/common/common.typ"
#import "/_src/mod.typ": *
#set heading(numbering: "1.")

#show raw.where(block: false): excerpt.inline.with(
  lang: "bash"
)

// Display block code in a larger block
// with more padding.
#show raw.where(block: true): excerpt.code.with(
  lang: "bash"
)
...
updating the code (e.g. after adding more action models), see #link("setup.html#Generate Docker Container")[Generate Docker Container]. 
...
See #link("users.html")[For Users] for instructions on how to use the web application.
...
or download its source code from #link("https://github.com/joint-probability-trees/jpt-dev")[GitHub.]
...
action plans. See sections :ref:`Reasoning` and :ref:`Plan Refinement` in #link("devs.html")[For Developers] for examples and the API
...
to start it. As described above, the webapp can then be accessed via web browser using the address #link("http://127.0.0.1:5005/bayrob/").

See #link("users.html")[For Users] for more information and a user manual of the web app. The added models should now appear in the
