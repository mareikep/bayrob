...
in [Use Web App in Docker](#use-web-app-in-docker) to start the application in a docker container. To inspect and use 
the code in your own project, follow the instructions in [Compile from Source](#compile-from-source). To generate and run your own docker container after
...
[http://127.0.0.1:5005/bayrob/](http://127.0.0.1:5005/bayrob/)

See [For Users](../user/intro) for instructions on how to use the web application.
...
action plans. See sections :ref:`Reasoning` and :ref:`Plan Refinement` in [For Developers](../dev/intro) for examples and the API
...
If additional models were integrated into the system (following the instructions in [Learning](../dev/learning)), the Docker
...
See [For Users](../user/intro) for more information and a user manual of the web app. The added models should now appear in the
